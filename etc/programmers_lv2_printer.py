...
def solution(priorities, location):
    ...
    while deq:
        ...
        # 튜플들간의 크기 비교에 있어서 튜플내의 첫번째 요소를 비교한 후 같은 경우 그 다음 번째 요소를 비교하며 최종적인 크기 비교를 진행한다. <br/> 따라서 반복문을 진행 중에 모든 튜플의 첫번째 요소가 같은 경우 차례차례 지워나가야 하지만 튜플 비교 원칙에 따라 그 다음 요소를 비교하는 경우가 생기므로 (Ex.(1,2)>(1,1)) "or deq[0][0] == max(deq)[0]" 코드를 삽입하여 해당 경우를 미연에 방지하였다.
